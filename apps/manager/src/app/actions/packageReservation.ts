        ...
        for (const item of items) {
            ...
            switch (item.service_type) {
                ...
                case 'tour':
                    ...
                    // 인원수와 정확히 일치하거나, 인원수 이상인 것 중 가장 작은 것 선택
                    const { data: tourPriceData } = await supabase
                        .from('tour_pricing')
                        .select('pricing_id, vehicle_type, price_per_person, tour:tour_id!inner(tour_name, tour_code)')
                        .eq('tour.tour_name', tourName)
                    ...
                    // 못 찾으면 가장 큰 인원수 검색
                    let tourPriceCode = tourPriceData?.pricing_id;
                    let tourVehicle = tourPriceData?.vehicle_type;

                    if (!tourPriceCode) {
                        const { data: fallbackTourPrice } = await supabase
                            .from('tour_pricing')
                            .select('pricing_id, vehicle_type, tour:tour_id!inner(tour_name)')
                            .eq('tour.tour_name', tourName)
                            .eq('is_active', true)
                            .order('max_guests', { ascending: false })
                            .limit(1)
                            .single();
                        tourPriceCode = fallbackTourPrice?.pricing_id;
                        tourVehicle = fallbackTourPrice?.vehicle_type;
                    }
