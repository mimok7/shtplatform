    ...
    const loadCruiseReservations = async () => {
        try {
            ...
            if (skippedLegacyCodes.length > 0) {
                console.debug('ℹ️ cruise_rate_card 레거시 코드 제외:', skippedLegacyCodes);
            }
            ...
            const merged: CruiseReservation[] = reIds.map((reId: string) => {
                const r = reservationMap[reId] || { re_id: reId }; // reservation 정보 (없으면 기본값)
                const cItems = cruiseMap[reId] || []; // List of cruise rows
                const q = r.re_quote_id ? quoteMap[r.re_quote_id] : undefined;
                const u = r.re_user_id ? userMap[r.re_user_id] : {};

                // cruise_document에서 승선코드 이미지 가져오기
                ...
                // Representative data (use first item if available)
        ...
        try {
            ...
            if (hasImage) {
                ...
                if (userId) {
                    ...
                    // checkout_date: 해당 예약의 체크인 날짜 + 2일
