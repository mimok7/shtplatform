...
function PackageBookingContent() {
    ...
    // SHT 좌석 선택 핸들러
    const handleShtSeatSelect = useCallback((seatInfo: { vehicle: string; seat: string; category: string; usageDate?: string }) => {
        ...
        if (shtModalType === 'pickup') {
            ...
            if (cruiseItemId) {
                setItemDetails(prev => ({
                    ...
                    [cruiseItemId]: {
                        ...prev[cruiseItemId],
                        shtPickupSeat: seatInfo.seat,
                        shtPickupVehicle: seatInfo.vehicle,
                        shtPickupDate: seatInfo.usageDate || prev[cruiseItemId]?.shtPickupDate
                    }
                }));
            }
        } else {
            ...
            if (cruiseItemId) {
                setItemDetails(prev => ({
                    ...
                    [cruiseItemId]: {
                        ...prev[cruiseItemId],
                        shtDropoffSeat: seatInfo.seat,
                        shtDropoffVehicle: seatInfo.vehicle,
                        shtDropoffDate: seatInfo.usageDate || prev[cruiseItemId]?.shtDropoffDate
                    }
                }));
            }
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 pb-12">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {selectedPackage && (
                            <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-white/40 shadow-xl p-6">
                                ...
                                <div className="space-y-2">
                                    {selectedPackage.items?.sort((a: any, b: any) => {
                                        const getOrder = (item: any) => {
                                            const desc = (item.description || '').toLowerCase();
                                            if (item.service_type === 'airport') return 1;
                                            if (item.service_type === 'tour' && (desc.includes('닌빈') || desc.includes('ninh binh'))) return 2;
                                            if (item.service_type === 'cruise') return 3;
                                            if (item.service_type === 'tour' && (desc.includes('하노이') || desc.includes('hanoi'))) return 4;
                                            return 10 + (item.item_order || 0);
                                        };
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                                                ...
                                                                <div className="md:col-span-2">
                                                                    ...
                                                                    <input
                                                                        type="text"
                                                                        required
                                                                        placeholder="하차 위치 (영문 대문자)"
                                                                        className="w-full bg-white border-gray-200 rounded-lg py-1.5 px-3 text-sm font-bold placeholder:text-gray-400 focus:ring-1 focus:ring-green-500"
                                                                        value={itemDetails[item.id]?.accommodation || ''}
                                                                        onChange={(e) => setItemDetails({
                                                                            ...itemDetails,
                                                                            [item.id]: { ...itemDetails[item.id], accommodation: e.target.value.toUpperCase().replace(/[^A-Z0-9\s]/g, '') }
                                                                        })}
                                                                    />
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => setItemDetails({
                                                                            ...itemDetails,
                                                                            [item.id]: { ...itemDetails[item.id], sandingPickupLocation: e.target.value.toUpperCase().replace(/[^A-Z0-9\s]/g, '') }
                                                                        })}
                                                                    />
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <input
                                                                        type="text"
                                                                        required
                                                                        placeholder="드랍 위치 (영문 대문자)"
                                                                        className="w-full bg-white border-gray-200 rounded-lg py-1.5 px-3 text-sm font-bold placeholder:text-gray-400 focus:ring-1 focus:ring-blue-500"
                                                                        value={itemDetails[item.id]?.roomType || ''}
                                                                        onChange={(e) => setItemDetails({
                                                                            ...itemDetails,
                                                                            [item.id]: { ...itemDetails[item.id], roomType: e.target.value.toUpperCase().replace(/[^A-Z0-9\s]/g, '') }
                                                                        })}
                                                                    />
                                                                </div>
                                                            </div>
            ...
            {isShtCarModalOpen && (
                <ShtCarSeatMap
                    ...
                    pickupLocation={cruiseItemId ? itemDetails[cruiseItemId]?.accommodation : undefined}
                    dropoffLocation={cruiseItemId ? itemDetails[cruiseItemId]?.roomType : undefined}
                />
            )}
        </div>
    );
