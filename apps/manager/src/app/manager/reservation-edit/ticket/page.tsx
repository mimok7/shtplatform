    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (!updatedData || updatedData.length === 0) {
                const { error: insertError } = await supabase.from('reservation_ticket').insert({ reservation_id: reservationId, ...payload });
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">성인 단가 ({currencyLabel})</label>
                                    <input
                                        type="number"
                                        value={formData.adult_unit_price}
                                        onChange={(e) => setFormData((prev) => recalcDerived({ ...prev, adult_unit_price: Number(e.target.value) || 0 }))}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                                    />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">아동 단가 ({currencyLabel})</label>
                                    <input
                                        type="number"
                                        value={formData.child_unit_price}
                                        onChange={(e) => setFormData((prev) => recalcDerived({ ...prev, child_unit_price: Number(e.target.value) || 0 }))}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                                    />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">셔틀 단가 ({currencyLabel})</label>
                                    <input
                                        type="number"
                                        value={formData.shuttle_unit_price}
                                        onChange={(e) => setFormData((prev) => recalcDerived({ ...prev, shuttle_unit_price: Number(e.target.value) || 0 }))}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-sm text-blue-700">
                                요금표 매칭: {priceInfoText}
                                <div className="text-xs text-blue-600 mt-1">대표 코드: {formData.ticket_price_code || '미매칭'}</div>
                            </div>
