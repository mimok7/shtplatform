  ...
  const loadDetailedServices = async () => {
    try {
      ...
      quoteItems?.forEach(item => {
        if (item.service_ref_id && serviceIds[item.service_type]) {
          serviceIds[item.service_type].push(item.service_ref_id);
        }
      });

      // 2. 서비스 상세 정보 일괄 조회 (병렬 처리)
      const serviceDataMap: { [key: string]: any[] } = {};
      ...
      await Promise.all(
        Object.keys(serviceIds).map(async (type) => {
          const ids = serviceIds[type];
          if (ids.length === 0) return;

          let query = supabase.from(type).select('*').in('id', ids);

          const { data, error } = await query;
          if (error) {
            console.error(`${type} 일괄 조회 실패:`, error);
            return;
          }

          serviceDataMap[type] = data || [];

          // 코드를 수집하여 가격 조회 준비
          data?.forEach((item: any) => {
            const codeField = `${type === 'rentcar' ? 'rentcar' : type}_code`;
            if (item[codeField]) {
              serviceCodeMap[type].push(item[codeField]);
            }
          });
        })
      );
      ...
      await Promise.all(
        Object.keys(serviceCodeMap).map(async (type) => {
          const codes = serviceCodeMap[type];
          ...
          switch (type) {
            ...
            case 'tour': tableName = 'tour_pricing'; codeField = 'pricing_id'; break;
            default: return;
          }
          ...
          if (error) {
            console.error(`${type} 가격 조회 실패:`, error);
            return;
          }

          priceDataMap[type] = data || [];
        })
      );
      ...
      for (const item of quoteItems || []) {
        try {
          ...
          const refId = item.service_ref_id;

          if (!refId || !serviceDataMap[type]) continue;

          // 해당 서비스 정보 찾기
          const serviceInfo = serviceDataMap[type].find((s: any) => s.id === refId);

          if (serviceInfo) {
            const codeField = `${type === 'rentcar' ? 'rentcar' : type}_code`; // rentcar만 예외 처리
            const code = serviceInfo[codeField];

            // 해당 가격 정보 찾기
            let priceInfo: any[] = [];
            if (priceDataMap[type]) {
              const priceCodeField = type === 'rentcar' ? 'rent_code' : codeField; // 가격 테이블의 코드 컬럼명
              priceInfo = priceDataMap[type].filter((p: any) => p[priceCodeField] === code);
            }

            // 상세 객체에 추가
            switch (type) {
              case 'room':
                detailed.rooms.push({ ...item, roomInfo: serviceInfo, priceInfo });
                break;
              case 'car':
                detailed.cars.push({ ...item, carInfo: serviceInfo, priceInfo });
                break;
              case 'airport':
                detailed.airports.push({ ...item, airportInfo: serviceInfo, priceInfo });
                break;
              case 'hotel':
                detailed.hotels.push({ ...item, hotelInfo: serviceInfo, priceInfo });
                break;
              case 'rentcar':
                detailed.rentcars.push({ ...item, rentcarInfo: serviceInfo, priceInfo });
                break;
              case 'tour':
                detailed.tours.push({ ...item, tourInfo: serviceInfo, priceInfo });
                break;
            }
          }
        } catch (mapError) {
          console.warn(`데이터 매핑 중 오류:`, mapError);
        }
      }
