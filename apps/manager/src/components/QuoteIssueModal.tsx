...
function QuoteFooter() {
  return <footer className="manager-quote-footer mt-5 grid gap-5 border-t border-slate-200 pt-3 text-xs text-slate-500 sm:grid-cols-2 sm:gap-8">
    <div className="grid gap-3"><div><strong className="block text-slate-900">STAY HALONG</strong><span>하롱베이 현지 프리미엄 여행</span></div><p>본 견적서는 예약 확정서가 아닙니다.<br />최종 금액과 예약 가능 여부는 결제 전 다시 확인합니다.<br />견적서의 유효기간은 발생일로 부터 2일간 입니다.</p></div>
    <div className="grid gap-1 text-left leading-5 sm:text-right"><strong className="text-slate-900">🏢 스테이하롱 트레블 | 하롱베이 상주 한국인 베트남 전문 여행사</strong><span>📍 상호 : CONG TY TENPER COMMUNICATIONS</span><span>📍 주소 : PHUONG YET KIEU, THANH PHO HA LONG</span><span>📧 support@stayhalong.com | ☎️ 07045545185 |</span><span>🌐 <a href="https://stayhalong.com" target="_blank" rel="noreferrer" className="underline">https://stayhalong.com</a> &nbsp; <a href="https://cafe.naver.com/stayhalong" target="_blank" rel="noreferrer" className="underline">https://cafe.naver.com/stayhalong</a></span><span>🕒 운영시간: 평일 09:00-24:00 (토요일 09:00-15:00, 일요일/공휴일 비상업무)</span><span>© 2024 StayHalong Travel. All rights reserved.</span></div>
  </footer>;
}
...
export default function QuoteIssueModal({ quoteId, quoteTitle, rows, totalDong, totalWon, defaultRecipient, defaultMemo, getAccessToken }: {
    ...
    const loadLatest = async () => {
      try {
        ...
        if (!cancelled && result.issue) {
          ...
          setRecipient(recipientLabel(result.issue.recipient_name || defaultRecipient || quoteTitle));
  ...
  const openModal = () => {
    setIssue(null);
    setRecipient(recipientLabel(defaultRecipient || quoteTitle));
    setMemo(defaultMemo);
    setOpen(true);
  };
  ...
  const issueQuote = async () => {
    ...
    try {
      ...
      setIssue(result.issue);
      setRecipient(recipientLabel(result.issue.recipient_name));
