...
export default function QuoteIssueModal({ quoteId, quoteTitle, rows, totalDong, totalWon, defaultRecipient, defaultMemo, getAccessToken }: {
  ...
  return <><style>{`.manager-quote-document footer::after{content:"🌊 스테이하롱 트레블과 함께하는 특별한 여행 🌊\\A 베트남 하롱베이에서 잊지 못할 추억을 만들어보세요!\\A 🏢 스테이하롱 트레블 | 하롱베이 상주 한국인 베트남 전문 여행사\\A 📍 상호 : CONG TY TENPER COMMUNICATIONS\\A 📍 주소 : PHUONG YET KIEU, THANH PHO HA LONG\\A 📧 support@stayhalong.com | ☎️ 07045545185 | 🌐 https://cafe.naver.com/stayhalong\\A 🕒 운영시간: 평일 09:00-24:00 (토요일 09:00-15:00, 일요일/공휴일 비상업무)\\A © 2024 StayHalong Travel. All rights reserved.";display:block;order:3;width:100%;margin-top:16px;padding-top:16px;border-top:1px solid #cbd5e1;white-space:pre-line;color:#475569;font-size:11px;line-height:1.75;text-align:left}`}</style>
    ...
    {open && <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/55 p-4 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="manager-quote-issue-title">
      <div className="w-full max-w-3xl rounded-xl bg-white p-5 shadow-2xl">
        <div className="no-print flex items-start justify-between gap-4 border-b border-slate-200 pb-4"><div><p className="text-xs font-semibold tracking-wider text-emerald-700">QUOTATION / SAVED ITEMS</p><h2 id="manager-quote-issue-title" className="mt-1 text-2xl font-bold text-slate-900">견적서 발행</h2></div><button type="button" onClick={() => setOpen(false)} className="rounded border border-slate-300 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">닫기</button></div>
