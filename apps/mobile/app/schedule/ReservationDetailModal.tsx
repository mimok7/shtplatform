    ...
    const enrich = async () => {
      ...
      try {
        ...
        const rentCodes = groupedItems
          .filter((s) => (s.serviceType === 'rentcar' && s.rentcar_price_code) || ((s.serviceType === 'vehicle' || s.serviceType === 'car') && (s.rentcar_price_code || s.car_price_code)))
          ...
          <div className="mb-2 flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <h2 className="truncate whitespace-nowrap text-base font-semibold text-gray-900">예약 통합 상세</h2>
                {item.source === 'sh' && <span className="rounded-full bg-gray-200 px-2 py-0.5 text-[11px] font-medium text-gray-700 whitespace-nowrap">Old</span>}
                {item.source === 'new' && <span className="rounded-full bg-blue-100 px-2 py-0.5 text-[11px] font-medium text-blue-700 whitespace-nowrap">New</span>}
              </div>
              <p className="truncate text-xs text-gray-500">{groupKey} · 연결 서비스 {enrichedServices.length}건</p>
            </div>
