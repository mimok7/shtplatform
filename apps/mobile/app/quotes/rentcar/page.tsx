...
import RentcarFormLite from '@/components/RentcarFormLite';
...
function RentcarQuoteContentManager() {
    const router = useRouter();
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-2">
                {quoteId ? (
                    <RentcarFormLite quoteId={quoteId} onSuccess={onFormSuccess} />
                ) : (
...
function RightDetailsCard() {
    ...
    // 렌트카 서비스의 총금액만 계산
    const rentcarTotal = (detailedServices.rentcars || []).reduce((s: number, rc: any) => s + (rc.item?.total_price || 0), 0);
            ...
            <div ref={naturalRef} className="mt-2">
                <div ref={naturalTextRef} className="text-sm text-gray-700 whitespace-pre-wrap">
                    {(() => {
                        ...
                        // 렌트카 서비스 표시
                        if (detailedServices.rentcars?.length) {
                            detailedServices.rentcars.forEach((rc: any) => {
                                const p = rc.priceInfo?.[0];
                                const unit = p?.price ?? p?.base_price ?? rc.item?.unit_price ?? 0;
                                const cnt = rc.item?.quantity ?? 1;
                                const tot = rc.item?.total_price ?? (unit * cnt);

                                out += `렌트카(${p?.way_type || ''})\n`;
                                out += `경로: ${p?.route || ''}\n`;
                                out += `차량: ${p?.vehicle_type || ''}\n`;
                                out += `1대 ${formatDong(unit)} * ${cnt}대 = ${formatDong(tot)}\n\n`;
                            });
                        }

                        out += `총금액: ${formatDong(rentcarTotal)}\n`;
                        // 정확한 환율 계산: 동화 금액을 환율로 나누기
                        const won = roundKrwToHundred(vndToKrw(rentcarTotal, EXCHANGE_RATE));
...
export default function ManagerRentcarQuotePage() {
    return (
        <Suspense fallback={
            ...
            <MobileQuoteLayout title="견적 입력">
                <ManagerServiceTabs active="rentcar" />
                <RentcarQuoteContentManager />
            </MobileQuoteLayout>
        </Suspense>
    );
}
