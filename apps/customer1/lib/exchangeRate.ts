export * from './exchangerate_impl';

// Compatibility barrel: re-export implementation from exchangerate_impl.
// 환율 관리 유틸리티
export interface ExchangeRate {
    currency_code: string;
    // rate_to_krw is the canonical admin-provided value interpreted as
    // KRW per 100 VND (관리자 입력값: 100동 당 원화). The canonical
    // conversion used across the app is: KRW = VND * rate_to_krw * 0.01
    rate_to_krw: number;
    last_updated: string;
    source: string;
}

// 캐시를 위한 변수 (현재 미사용하지만 보존)
...
/**
 * 최신 환율을 조회합니다.
 * 이 함수는 서버(DB)에 저장된 관리자가 입력한 canonical 환율만을 사용합니다.
 * 클라이언트 로컬(fallback)이나 외부 네이버 API 자동 대체는 제거되었습니다.
 */
...
export function formatExchangeRate(rate: number): string {
    // Present the canonical admin-entered rate as '100동 = X원'.
    // The admin inputs rate as KRW per 100 VND (예: 5.25), so showing
    // '100동 = 5.25원' is the clearest representation for managers.
