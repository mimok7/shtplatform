...
const CACHE_NAME = 'sht-partner-cache-v1';
...
self.addEventListener('push', event => {
  let data = { title: '스테이하롱 알림', body: '새 알림이 도착했습니다', url: '/partner/dashboard' };
  try {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title, {
      body: data.body,
      icon: data.icon || '/sht-partner.png',
      badge: data.badge || '/sht-partner.png',
      tag: data.tag || 'sht-notification',
      data: { url: data.url || '/partner/dashboard' },
