...
export async function POST(
    ...
    try {
        ...
        // 차량 서비스 (레거시 + ops 병렬 조회)
        const [legacyCarRes, opsCarRes] = await Promise.all([
            supabase
        ...
        const carShtData = [...(legacyCarRes.data || []), ...(opsCarRes.data || [])];

        // 중복 제거: id 기준
        const carShtDataDedup = carShtData.reduce((acc, item) => {
        ...
        if (carShtDataDedup && carShtDataDedup.length > 0) {
            services.push({ type: 'car_sht', name: '차량', data: carShtDataDedup });
        }
