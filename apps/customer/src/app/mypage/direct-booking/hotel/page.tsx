...
function HotelDirectBookingContent() {
    ...
    // 사용자 인증 (MyPageLayout이 인증 가드를 담당. 여기서는 user state만 동기화)
    useEffect(() => {
        let cancelled = false;
        ...
        supabase.auth.getSession().then(({ data: { session } }) => {
            ...
            if (session?.user) {
                ...
                if (editMode) loadExistingHotelReservation(session.user.id);
            }
        });
    ...
    const loadHotelNameOptions = useCallback(async () => {
        try {
            // 1. hotel_info에서 활성 호텔 목록 조회 (날짜 무관)
    ...
    const loadRoomCards = useCallback(async (hotelName: string) => {
        try {
            // hotel_price에서 해당 호텔의 객실 조회 (날짜/요일 무관)
