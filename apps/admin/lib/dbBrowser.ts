...
export async function insertTableRow(table: string, values: Record<string, unknown>, source: DbDataSource = 'platform') {
  ...
  for (const [key, value] of Object.entries(values || {})) {
    ...
    if (isAutoGeneratedColumn(columns.find((column) => column.column_name === key)!)) continue;
...
export async function updateTableRow(table: string, keyValue: unknown, values: Record<string, unknown>, source: DbDataSource = 'platform') {
  ...
  for (const [name, value] of Object.entries(values || {})) {
    ...
    if (name === key.column_name || isAutoGeneratedColumn(columns.find((column) => column.column_name === name)!)) throw new Error('자동 생성 컬럼은 수정할 수 없습니다.');
    payload[name] = value;
  }
