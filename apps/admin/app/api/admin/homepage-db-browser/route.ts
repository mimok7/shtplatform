...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const body = await request.json();
    const table = typeof body?.table === 'string' ? body.table.trim() : '';
    const result = await updateTableRow(table, body?.keyValue, body?.values && typeof body.values === 'object' ? body.values : {}, 'homepage');
    return NextResponse.json({ ok: true, ...result });
  } catch (error: unknown) {
    return NextResponse.json({ error: error instanceof Error ? error.message : '홈페이지 DB 행 수정에 실패했습니다.' }, { status: 400 });
  }
}
