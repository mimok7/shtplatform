...
import { ArrowDown, ArrowUp, ArrowUpDown, Columns3, Copy, Download, ExternalLink, Pencil, Plus, RefreshCw, Search, X } from 'lucide-react';
...
type DatabaseSource = 'platform' | 'homepage';

const DATABASE_CONFIG = {
  platform: { apiPath: '/api/admin/db-browser', exportMode: 'table', layoutTitle: 'DB 관리', pageTitle: '플랫폼 DB 테이블 관리', activeTab: 'database' },
  homepage: { apiPath: '/api/admin/homepage-db-browser', exportMode: 'homepage_table', layoutTitle: '홈페이지 DB 관리', pageTitle: '홈페이지 DB 테이블 관리', activeTab: 'homepage-database' },
} as const;
...
function parseValue(column: DbColumn, value: string): unknown {
  ...
  if (column.data_type === 'ARRAY') {
    try {
      ...
      return parsed;
    } catch { throw new Error(`${column.column_name} 배열 JSON 형식을 확인해 주세요.`); }
  }
...
function rowKeyColumn(columns: DbColumn[]) {
  ...
  if (id) return id.column_name;
  const idColumns = columns.filter((column) => column.column_name.endsWith('_id'));
  if (idColumns.length === 1) return idColumns[0].column_name;
  return columns.find((column) => ['code', 'key', 'setting_key'].includes(column.column_name))?.column_name || '';
}

function isAutoGeneratedColumn(column: DbColumn) {
  return column.column_name === 'id'
    || Boolean(column.column_default)
    || Boolean(column.identity_generation)
    || ['created_at', 'updated_at', 'deleted_at', 'inserted_at', 'modified_at'].includes(column.column_name);
}

function formValue(column: DbColumn, value: unknown) {
  if (value === null || value === undefined) return '';
  if (column.data_type === 'boolean') return value ? 'true' : 'false';
  if (column.data_type === 'json' || column.data_type === 'jsonb' || column.data_type === 'ARRAY') return JSON.stringify(value);
...
export function DatabaseManagementPage({ source = 'platform' }: { source?: DatabaseSource }) {
  const database = DATABASE_CONFIG[source];
  ...
  const loadTables = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${database.apiPath}?mode=tables`, { headers: await authHeaders(), cache: 'no-store' });
  ...
  const openAdd = (sourceRow?: DbRow | MouseEvent<HTMLButtonElement>) => {
    const copiedRow = sourceRow && !('currentTarget' in sourceRow) ? sourceRow : undefined;
    const initial: Record<string, string> = {};
    columns.forEach((column) => { initial[column.column_name] = copiedRow && !isAutoGeneratedColumn(column) ? formValue(column, copiedRow[column.column_name]) : column.data_type === 'boolean' ? 'false' : ''; });
    setForm(initial);
    setAddMode(copiedRow ? 'copy' : 'new');
    setAddOpen(true);
  };

  const keyColumn = useMemo(() => rowKeyColumn(columns), [columns]);
  const openEdit = (row: DbRow) => {
    const initial: Record<string, string> = {};
    columns.forEach((column) => { initial[column.column_name] = formValue(column, row[column.column_name]); });
    setForm(initial);
    setEditingRow(row);
  };
  ...
  const addRow = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(database.apiPath, { method: 'POST', headers: { ...await authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ table: selectedTable, values }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || '행 추가에 실패했습니다.');
      setAddOpen(false);
      setNotice({ type: 'success', text: addMode === 'copy' ? `${selectedTable} 복사본을 새 행으로 추가했습니다.` : `${selectedTable} 테이블에 행을 추가했습니다.` });
  ...
  const updateRow = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!editingRow || !keyColumn) return;
    setSaving(true);
    setNotice(null);
    try {
      const values: Record<string, unknown> = {};
      for (const column of columns) {
        if (column.column_name === keyColumn || isAutoGeneratedColumn(column)) continue;
        values[column.column_name] = parseValue(column, form[column.column_name] ?? '');
      }
      const keyValue = editingRow[keyColumn];
      const response = await fetch(database.apiPath, { method: 'PATCH', headers: { ...await authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ table: selectedTable, keyValue, values }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || '행 수정에 실패했습니다.');
      const updated = result.data as DbRow;
      setRows((current) => current.map((row) => row[keyColumn] === keyValue ? updated : row));
      setEditingRow(null);
      setNotice({ type: 'success', text: `${selectedTable} 행을 수정했습니다.` });
    } catch (error: unknown) {
      setNotice({ type: 'error', text: error instanceof Error ? error.message : '행 수정에 실패했습니다.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const exportTable = async () => {
    ...
    setNotice(null);
    try {
      const response = await fetch('/api/admin/sheets-sync', { method: 'POST', headers: { ...await authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: database.exportMode, table: selectedTable, search: rowSearch, filters: columnFilters, sort: sort.column, direction: sort.direction }) });
      const result = await response.json().catch(() => ({}));
  ...
  return (
    <AdminLayout title={database.layoutTitle} activeTab={database.activeTab}>
      ...
      {editingRow && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-labelledby="db-edit-title"><form onSubmit={updateRow} className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden border border-[var(--sht-border)] bg-[var(--sht-surface)] shadow-xl"><header className="flex items-center justify-between border-b border-[var(--sht-border)] px-5 py-4"><div><h2 id="db-edit-title" className="font-semibold text-[var(--sht-heading)]">{selectedTable} 행 수정</h2><p className="mt-1 text-xs text-[var(--sht-text-muted)]">식별 키 {keyColumn}은 변경할 수 없습니다.</p></div><button type="button" title="닫기" aria-label="닫기" onClick={() => setEditingRow(null)} className="flex h-11 w-11 items-center justify-center text-[var(--sht-text-muted)] hover:bg-[var(--sht-surface-muted)]"><X size={18} /></button></header><div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-y-auto p-5 md:grid-cols-2">{columns.map((column) => { const locked = column.column_name === keyColumn; const multiline = column.data_type === 'json' || column.data_type === 'jsonb' || column.data_type === 'ARRAY'; return <label key={column.column_name} className="grid gap-1.5 text-xs font-semibold text-[var(--sht-text-muted)]">{column.column_name}{locked && <span>식별 키</span>}{column.data_type === 'boolean' ? <select value={form[column.column_name] ?? 'false'} disabled={locked} onChange={(event) => setForm((current) => ({ ...current, [column.column_name]: event.target.value }))} className="h-11 border border-[var(--sht-border)] bg-[var(--sht-surface)] px-3 text-sm font-normal text-[var(--sht-text)] focus:outline-none focus:ring-2 focus:ring-[var(--sht-focus)] disabled:bg-[var(--sht-surface-muted)]"><option value="true">true</option><option value="false">false</option></select> : multiline ? <textarea value={form[column.column_name] ?? ''} disabled={locked} onChange={(event) => setForm((current) => ({ ...current, [column.column_name]: event.target.value }))} rows={5} className="border border-[var(--sht-border)] bg-[var(--sht-surface)] p-3 font-mono text-xs font-normal text-[var(--sht-text)] focus:outline-none focus:ring-2 focus:ring-[var(--sht-focus)] disabled:bg-[var(--sht-surface-muted)]" /> : <input type={inputType(column)} value={form[column.column_name] ?? ''} disabled={locked} onChange={(event) => setForm((current) => ({ ...current, [column.column_name]: event.target.value }))} className="h-11 border border-[var(--sht-border)] bg-[var(--sht-surface)] px-3 text-sm font-normal text-[var(--sht-text)] focus:outline-none focus:ring-2 focus:ring-[var(--sht-focus)] disabled:bg-[var(--sht-surface-muted)]" />}</label>; })}</div><footer className="flex justify-end gap-2 border-t border-[var(--sht-border)] bg-[var(--sht-surface-muted)] px-5 py-4"><button type="button" onClick={() => setEditingRow(null)} className="h-11 border border-[var(--sht-border)] px-4 text-sm font-semibold text-[var(--sht-text)]">취소</button><button type="submit" disabled={saving} className="h-11 bg-[var(--sht-primary)] px-5 text-sm font-semibold text-[var(--sht-primary-text)] disabled:opacity-50">{saving ? '저장 중...' : '행 저장'}</button></footer></form></div>}
    </AdminLayout>
  );
}

export default function PlatformDatabaseManagementPage() {
