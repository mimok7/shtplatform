...
export default function DatabaseManagementPage() {
  ...
  const loadRows = useCallback(async (table: string, search: string, nextPage: number) => {
    if (!table) return;
    setLoadingRows(true);
    try {
      const params = new URLSearchParams({ mode: 'rows', table, search, offset: String(nextPage * 100), limit: '100' });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { void loadRows(selectedTable, rowSearch, 0); }, 250);
    return () => window.clearTimeout(timer);
  }, [loadRows, rowSearch, selectedTable]);
  ...
  const addRow = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await loadRows(selectedTable, rowSearch, 0);
    } catch (error: unknown) {
  ...
  const exportTable = async () => {
    ...
    setNotice(null);
    try {
      const response = await fetch('/api/admin/sheets-sync', { method: 'POST', headers: { ...await authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: 'table', table: selectedTable, search: rowSearch }) });
      const result = await response.json().catch(() => ({}));
    ...
    <AdminLayout title="DB 관리" activeTab="database">
      <div className="space-y-4" data-sht-app="admin" data-sht-theme="default">
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <section className="min-w-0 border border-[var(--sht-border)] bg-[var(--sht-surface)]"><div className="flex flex-col gap-3 border-b border-[var(--sht-border)] p-4 md:flex-row md:items-center md:justify-between"><div><h2 className="font-semibold text-[var(--sht-heading)]">{selectedTable || '테이블을 선택하세요'}</h2><p className="mt-1 text-xs text-[var(--sht-text-muted)]">{totalRows.toLocaleString('ko-KR')}건 · 1페이지 100건</p></div><div className="flex flex-wrap justify-end gap-2">{selectedTable && <><button type="button" onClick={openAdd} disabled={loadingRows || saving || columns.length === 0} className="inline-flex h-11 items-center gap-2 bg-[var(--sht-primary)] px-4 text-sm font-semibold text-[var(--sht-primary-text)] hover:bg-[var(--sht-primary-hover)] disabled:opacity-50"><Plus size={16} />행 추가</button><button type="button" onClick={() => void exportTable()} disabled={loadingRows || exporting} className="inline-flex h-11 items-center gap-2 bg-green-600 px-4 text-sm font-semibold text-white hover:bg-green-700 disabled:opacity-50"><Download size={16} />{exporting ? '내보내는 중...' : '구글시트 내보내기'}</button></>}</div></div>{selectedTable && <div className="border-b border-[var(--sht-border)] bg-[var(--sht-surface-muted)] p-4"><label className="relative block max-w-xl"><span className="sr-only">행 검색</span><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--sht-text-muted)]" /><input value={rowSearch} onChange={(event) => setRowSearch(event.target.value)} placeholder="현재 테이블의 문자열·코드·날짜 검색" className="h-11 w-full border border-[var(--sht-border)] bg-[var(--sht-surface)] pl-9 pr-3 text-sm text-[var(--sht-text)] focus:outline-none focus:ring-2 focus:ring-[var(--sht-focus)]" /></label></div>}<div className="overflow-x-auto"><table className="min-w-full border-collapse text-left text-xs"><thead className="bg-[var(--sht-surface-muted)]"><tr>{columns.map((column) => <th key={column.column_name} className="whitespace-nowrap border-b border-[var(--sht-border)] px-3 py-3 font-semibold text-[var(--sht-text-muted)]">{column.column_name}</th>)}</tr></thead><tbody>{loadingRows ? <tr><td colSpan={Math.max(columns.length, 1)} className="h-48 text-center text-sm text-[var(--sht-text-muted)]">행을 불러오는 중...</td></tr> : rows.length === 0 ? <tr><td colSpan={Math.max(columns.length, 1)} className="h-48 text-center text-sm text-[var(--sht-text-muted)]">데이터가 없습니다.</td></tr> : rows.map((row, index) => <tr key={`${selectedTable}-${index}`} className="border-b border-[var(--sht-border)] last:border-0 hover:bg-[var(--sht-surface-muted)]">{columns.map((column) => <td key={column.column_name} className="max-w-[280px] truncate whitespace-nowrap px-3 py-3 text-[var(--sht-text)]" title={formatValue(row[column.column_name])}>{formatValue(row[column.column_name])}</td>)}</tr>)}</tbody></table></div>{selectedTable && <div className="flex items-center justify-between border-t border-[var(--sht-border)] px-4 py-3"><button type="button" onClick={() => void loadRows(selectedTable, rowSearch, page - 1)} disabled={page === 0 || loadingRows} className="h-10 border border-[var(--sht-border)] px-3 text-sm text-[var(--sht-text)] disabled:opacity-40">이전</button><span className="text-xs text-[var(--sht-text-muted)]">{page + 1} / {Math.max(1, Math.ceil(totalRows / 100))}</span><button type="button" onClick={() => void loadRows(selectedTable, rowSearch, page + 1)} disabled={!hasMore || loadingRows} className="h-10 border border-[var(--sht-border)] px-3 text-sm text-[var(--sht-text)] disabled:opacity-40">다음</button></div>}</section>
        </div>
      </div>
