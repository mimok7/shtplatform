...
export default function DatabaseManagementPage() {
  ...
  const filteredTables = useMemo(() => {
    ...
    return query ? tables.filter((item) => `${item.table} ${describeTable(item.table)}`.toLowerCase().includes(query)) : tables;
  }, [tableSearch, tables]);
  ...
  return (
    <AdminLayout title="DB 관리" activeTab="database">
      <div className="space-y-3 pb-10" data-sht-app="admin" data-sht-theme="default">
        ...
        <section className="border border-[var(--sht-border)] bg-[var(--sht-surface)] p-3"><div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-semibold text-[var(--sht-heading)]">테이블 선택</h2><p className="mt-0.5 text-xs text-[var(--sht-text-muted)]">{filteredTables.length}/{tables.length}개 · 이름 또는 한글 설명으로 찾기</p></div><label className="relative block w-full sm:max-w-sm"><span className="sr-only">테이블 검색</span><Search size={14} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-[var(--sht-text-muted)]" /><input value={tableSearch} onChange={(event) => setTableSearch(event.target.value)} placeholder="테이블명 또는 설명 검색" className="h-9 w-full border border-[var(--sht-border)] bg-[var(--sht-surface)] pl-8 pr-2 text-xs text-[var(--sht-text)] focus:outline-none focus:ring-2 focus:ring-[var(--sht-focus)]" /></label></div><div className="grid max-h-64 grid-cols-1 gap-1.5 overflow-y-auto pr-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{loadingTables ? <div className="p-2 text-xs text-[var(--sht-text-muted)]">테이블 목록 로딩 중...</div> : filteredTables.map((item) => <button key={item.table} type="button" onClick={() => selectTable(item.table)} className={`border px-2.5 py-1.5 text-left ${selectedTable === item.table ? 'border-[var(--sht-primary)] bg-[var(--sht-surface-muted)]' : 'border-[var(--sht-border)] hover:bg-[var(--sht-surface-muted)]'}`}><span className="block truncate text-xs font-medium text-[var(--sht-text)]">{item.table}</span><span className="mt-0.5 block truncate text-[11px] text-[var(--sht-text-muted)]">{describeTable(item.table)} · 컬럼 {item.columnCount}개</span></button>)}{!loadingTables && filteredTables.length === 0 && <div className="p-2 text-xs text-[var(--sht-text-muted)]">검색 결과가 없습니다.</div>}</div></section>

        <section className="min-w-0 border border-[var(--sht-border)] bg-[var(--sht-surface)]"><div className="flex flex-col gap-2 border-b border-[var(--sht-border)] p-3 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-sm font-semibold text-[var(--sht-heading)]">{selectedTable || '테이블을 선택하세요'}</h2>{selectedTable && <p className="mt-0.5 text-xs text-[var(--sht-text-muted)]">{describeTable(selectedTable)} · {totalRows.toLocaleString('ko-KR')}건 · 머릿글에서 값 선택 필터·정렬</p>}</div><div className="flex flex-wrap justify-end gap-1.5">{selectedTable && <><button type="button" onClick={() => setColumnPickerOpen((open) => !open)} disabled={columns.length === 0} className="inline-flex h-9 items-center gap-1.5 border border-[var(--sht-border)] px-3 text-xs font-semibold text-[var(--sht-text)] hover:bg-[var(--sht-surface-muted)]"><Columns3 size={14} />컬럼 표시</button><button type="button" onClick={openAdd} disabled={loadingRows || saving || columns.length === 0} className="inline-flex h-9 items-center gap-1.5 bg-[var(--sht-primary)] px-3 text-xs font-semibold text-[var(--sht-primary-text)] hover:bg-[var(--sht-primary-hover)] disabled:opacity-50"><Plus size={14} />행 추가</button><button type="button" onClick={() => void exportTable()} disabled={loadingRows || exporting} className="inline-flex h-9 items-center gap-1.5 bg-green-600 px-3 text-xs font-semibold text-white hover:bg-green-700 disabled:opacity-50"><Download size={14} />{exporting ? '내보내는 중...' : '구글시트 내보내기'}<ExternalLink size={13} /></button></>}</div></div>
