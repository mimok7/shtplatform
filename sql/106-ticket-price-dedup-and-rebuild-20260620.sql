  ...
  SELECT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY
        ...
        tp.price_item,
        tp.valid_from,
        COALESCE(tp.valid_to, DATE '9999-12-31'),
        tp.sort_order
      ORDER BY
        CASE
          WHEN EXISTS (
            ...
            WHERE rt.ticket_price_code = tp.ticket_price_code
          ) THEN 0
          ELSE 1
        END,
        tp.created_at ASC,
        tp.ticket_price_code ASC
    ) AS rn
  FROM public.ticket_price tp
  WHERE tp.is_active = true
)
UPDATE public.ticket_price tp
SET
  is_active = false,
  updated_at = NOW(),
  notes = CONCAT(
    COALESCE(tp.notes, ''),
    CASE
      WHEN COALESCE(tp.notes, '') = '' THEN ''
      ELSE E'\n'
    END,
    '[auto-disabled] duplicate active ticket price slot cleaned on 2026-06-20'
  )
FROM ranked_duplicates rd
WHERE tp.id = rd.id
  AND rd.rn > 1;

CREATE UNIQUE INDEX IF NOT EXISTS uq_ticket_price_active_slot
...
WITH ticket_rows AS (
  SELECT
    ...
    COALESCE(rt.ticket_price_code, '') AS stored_ticket_price_code,
    COALESCE(rt.ticket_price_item, 'adult') AS stored_ticket_price_item,
...
ticket_rows_with_price AS (
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE tp.is_active = true
      ...
      AND (
        (tr.stored_ticket_price_item = 'adult' AND tp.ticket_price_code = tr.stored_ticket_price_code)
        OR (
          tp.ticket_type = tr.ticket_type
          AND tp.valid_from <= tr.usage_date
          AND (tp.valid_to IS NULL OR tp.valid_to >= tr.usage_date)
        )
      )
    ORDER BY
      CASE WHEN tp.ticket_price_code = tr.stored_ticket_price_code THEN 0 ELSE 1 END,
      tp.valid_from DESC,
      tp.sort_order ASC,
      tp.created_at ASC,
      tp.ticket_price_code ASC
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE tp.is_active = true
      ...
      AND (
        (tr.stored_ticket_price_item = 'child_under_1_2m' AND tp.ticket_price_code = tr.stored_ticket_price_code)
        OR (
          tp.ticket_type = tr.ticket_type
          AND tp.valid_from <= tr.usage_date
          AND (tp.valid_to IS NULL OR tp.valid_to >= tr.usage_date)
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE tp.is_active = true
      ...
      AND (
        (tr.stored_ticket_price_item = 'shuttle' AND tp.ticket_price_code = tr.stored_ticket_price_code)
        OR (
          tp.ticket_type = tr.ticket_type
          AND tp.valid_from <= tr.usage_date
          AND (tp.valid_to IS NULL OR tp.valid_to >= tr.usage_date)
