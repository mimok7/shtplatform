...
## SHT 플랫폼 프로젝트 작업 지침 (SHT Platform Project Guidelines)
**SHT 프로젝트** 또는 **sht 플랫폼**과 관련된 작업 요청을 받으면, 반드시 `C:\SHT-DATA\sht-platform` 폴더를 작업 기준(Workspace)으로 삼아야 합니다.
- **데이터베이스 스키마 참조**: 새로운 SQL 작성이나 데이터베이스 관련 작업을 수행할 때는 반드시 `C:\SHT-DATA\sht-platform\sql` 디렉토리 내의 기존 `.sql` 파일들과 `db.csv` 파일(또는 관련 csv 파일들)에 정의된 테이블 및 컬럼 명세를 확인하고 이를 준수하여 작성해야 합니다.
- 이전에 입력한 SQL과 CSV의 컬럼 정보를 기반으로 데이터 일관성을 유지하세요.

**크루즈 요금 SQL 표준 작성 방식 (안전한 스크립트 형태)**
- 요금 등 주요 데이터를 업데이트할 때는 기존 데이터를 `DELETE` 하지 않습니다.
- 반드시 `NOT EXISTS` 구문과 `WITH source_rows AS (...)` (CTE) 문법을 사용하여 중복 추가를 방지합니다.
- 전체 트랜잭션을 `BEGIN;`과 `COMMIT;`으로 감싸서 무결성을 보장합니다.
- 데이터 타입 오류 방지를 위해 명시적 형변환(`::numeric`, `DATE '...'`)을 필수적으로 사용합니다.
- 쿼리 실행 직후 `RETURNING`과 `SELECT`를 이용해 새로 추가된 행(Row) 데이터를 출력(표 형태)하도록 구성해야 합니다.
